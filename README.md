...
Verseform `0.2.1` is the latest immutable Windows Beta pre-release. Its clean workflow preserved and verified the exact Alpha upgrade baseline, then published the [release](https://github.com/SydFloyd/verseform/releases/tag/v0.2.1) only after the canonical suite, Alpha-to-patch preservation, clean offline install/uninstall, advisory, and license proofs passed on a clean Windows runner. Session testing subsequently found that expanding an earlier reference can leave a later reference previewable but unable to insert; the correction is in source and requires a new verified patch before tester retest. The public [Digital Bible Society ARC API](https://arc.dbs.org/docs) and bundled World English Bible remain the online and explicit offline scripture sources; detection remains entirely local.
