...
Verseform `0.2.1` is the corrected Windows Beta patch. Its clean workflow preserved and verified the exact Alpha upgrade baseline, then published the [new immutable pre-release](https://github.com/SydFloyd/verseform/releases/tag/v0.2.1) only after the canonical suite, Alpha-to-patch preservation, clean offline install/uninstall, advisory, and license proofs passed on a clean Windows runner. The public [Digital Bible Society ARC API](https://arc.dbs.org/docs) and bundled World English Bible remain the online and explicit offline scripture sources; detection remains entirely local.
