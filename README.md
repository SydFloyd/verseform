...
Distribution is complete; **first independent-user validation is still pending**. The 2026-09-05 review identified recovery-restore safety, keyboard reachability, and PDF-preview pagination gaps despite a passing automated suite. `VFM-130` and `VFM-140` now correct the first two gaps in source; the published `0.2.0` installer remains unchanged and retains those documented limits. [The preparation roadmap](outputs/verseform-roadmap.md#first-user-preparation-review--2026-09-05) records the evidence and the remaining `VFM-150`–`VFM-160` exit proofs. See the release notes for the corrected-source walkthrough and current known issues before using the published Beta for writing.
