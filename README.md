...
Distribution is complete; **first independent-user validation is still pending**. The 2026-09-05 review identified recovery-restore safety, keyboard reachability, and PDF-preview pagination gaps despite a passing automated suite. `VFM-130` now corrects recovery restore in source; the published `0.2.0` installer remains unchanged and retains that documented limit. [The preparation roadmap](outputs/verseform-roadmap.md#first-user-preparation-review--2026-09-05) records the evidence and remaining `VFM-140`–`VFM-160` exit proofs. See the release notes for current known issues before using the published Beta for writing.
