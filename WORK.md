...
- Stage: `VFM-100` engineering and automated proof are complete; the compact Beta interaction surface is awaiting only owner visual acceptance before the roadmap item is closed.
- Active item: `VFM-100` visual acceptance. Do not expand the slice while it is in review.
- Next item after acceptance: `VFM-110` — stabilize, exercise Alpha-to-Beta upgrade preservation, and release the Windows Beta.
- Baseline: the native title is the only visible Verseform/document identity; compact File/Edit/Help and semantic formatting groups lead directly into the writing surface. The Credits & Licenses overlay is a kernel-owned, F1-accessible local view of version, DBS thanks, effective-translation notice, bundled WEB provenance, and all generated dependency-license records.
...
- `npm run check` passed on 2026-09-03: thirty-eight pure/provider/kernel/controller/architecture tests (plus one intentionally skipped live smoke), twenty-six production-browser tests, eight native unit tests (plus one intentionally ignored live smoke), and six Windows smoke tests, with clean TypeScript, Rust formatting, Clippy, CSP/capability checks, and 660 locked dependency-license records.
- The new browser proof covers 1120px/100%, 960px/125%, and 780px/150% layouts, whole-group wrapping, 32px controls, no horizontal clipping, chrome stability after editing, F1/pointer parity, modal focus and return, zero-request local credits, hostile provider metadata escaping, the browser external-link fake, and zero axe A/AA violations. All prior Alpha document, recovery, scripture, output, keyboard, and walking journeys still pass against the intentional title-based dirty/saved contract.
...
- `npm run build:desktop` produced the VFM-100 review build as unsigned 4,216,031-byte `src-tauri/target/release/bundle/nsis/Verseform_0.1.0_x64-setup.exe` with SHA-256 `26799060229E6EE41F9877284B978A6D67B6B918A63004D78DF8D7899ABA7558`. The prior local lifecycle proof remains valid for the unchanged per-user packaging and uninstall boundaries.
