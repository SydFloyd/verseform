...
- Windows X-close now uses one native path through Verseform's Save/Discard/Cancel gate. The desktop no longer registers a competing browser unload blocker, and its local main-window capability narrowly permits the final force-close after Save or Discard. A rejected force-close remains visible without clearing dirty writing.
...
- The public `0.2.1` build contains the two earlier session blockers; `0.2.2` resolves those but predates the complete-reference correction. Use the verified `0.2.4` installer for the remaining independent-user gate.

## Release evidence

- [Clean Windows run `34136320391`](https://github.com/SydFloyd/verseform/actions/runs/34136320391) passed at commit `9c620aed5ec227945dc29447032fb5a00b1d59f1`: the durable Alpha check, canonical suite, unsigned NSIS build, exact 0.1.0-to-0.2.4 data-preserving upgrade, separate clean offline install/uninstall, npm and Rust advisory audits, artifact evidence, publication, and public re-download verification all succeeded. The canonical gate contained 63 pure/provider/kernel/controller/architecture/Windows-readiness tests plus one skipped live smoke, 44 desktop-browser journeys, 13 production-web journeys, 11 native unit tests plus one ignored live smoke, 7 Windows smoke tests, TypeScript, Rust formatting/Clippy, release boundaries, and 678 dependency-license records.
- The public [`v0.2.4` pre-release](https://github.com/SydFloyd/verseform/releases/tag/v0.2.4) and tag resolve to that exact commit. Its unsigned 4,310,783-byte installer has SHA-256 `019db8ce2bc0db6655d301faa47721b582b1c3de4d43da3d7eae809a527ea103`; a fresh independent public download returned the same size, digest, and `NotSigned` status. Artifact `verseform-0.2.4-windows-beta` is ID `10024840681` with archive SHA-256 `9fe0757820d0af5e6d8925ed3bdb4b0ffbddc96d19f5a065825de74e61f68ca2`. GitHub records `SHA256SUMS.txt` as `sha256:54993f67e5f208f561e9cd7f50d690be8977bbc9dfd1047e9d813a14ac9486cd` and `release-evidence.json` as `sha256:21c0c2348e815f87f4e9aaa35e42bb5ece3fcf25537122d5b8cf9c02d35964eb`.
...
- Initial run `34078849591` passed the application checks and built the installer, but stopped before patch installation because the retained Alpha was not responsive at the upgrade smoke's single six-second observation. No release was published from that run. Both installer launch checks now use bounded readiness retries, still fail on early exit or a sustained hang, and retain all data-preservation assertions. A focused Windows regression covers delayed readiness and failure cleanup; the successful retry above is the release authority.
