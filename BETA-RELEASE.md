...
- Initial run `34078849591` passed those checks and built the installer, but stopped before patch installation because the retained Alpha was not responsive at the upgrade smoke's single six-second observation. No release was published from that run. Both installer launch checks now allow up to 30 seconds for readiness, still fail on early exit or a sustained hang, and retain all data-preservation assertions. A focused Windows regression covers delayed readiness and failure cleanup; the successful retry is required for publication.
