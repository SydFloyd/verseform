...
Available authorized translations are loaded from DBS. The compact selector shows the active translation's citation abbreviation at rest and expands to a keyboard-accessible, searchable list that presents each abbreviation with its full title. The user's preferred translation is stored in a local device profile and used by default across documents. On a first online session with no saved preference, NASB is preferred when the DBS catalog offers it. Without connectivity, the editor clearly switches to the bundled WEB translation and identifies the fallback in previews and inserted citations; that temporary fallback does not overwrite the saved preference. If one online passage is unavailable or malformed while the catalog remains available, an explicit whole-passage WEB fallback keeps the selected online translation active so the next reference retries it.

Every supported 66-book reference must use the scripture provider's matching route and response identifiers. ARC chapter URLs retain Verseform's standard USFM book ID, while ARC response keys use DBS's two-character ID, including IDs with a trailing number. The DBS boundary must accept verses only when that response ID and chapter match the request. A verse returned by ARC, including numbered-book references such as `2 Peter 3:8`, must not fall back to WEB because of an identifier mismatch. If ARC omits a requested verse, Verseform may use its explicit whole-passage fallback but must not relabel WEB text as the selected translation.
