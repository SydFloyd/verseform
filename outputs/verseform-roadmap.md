...
| `VFM-060` | next | Harden and ship the Windows beta. Close accessibility and keyboard gaps, bound large-document behavior, handle low disk and interrupted writes, audit dependencies/licenses/capabilities/CSP, build the installer, and document privacy, supported behavior, and known limits. | A clean Windows machine installs, launches offline, completes detect-to-insert/save/reopen/print/PDF, and uninstalls without deleting user documents; the release checklist and focused regression suite pass. |
