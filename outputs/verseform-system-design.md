...
File owns document and output commands. Edit owns undo/redo, Find, and Paragraph. Help owns **Credits & Licenses**. File/Edit/Help form one quiet menu strip without disclosure arrows; command and formatting controls reserve their border geometry but reveal it only on hover, keyboard focus, or active state. Shared size, gap, border, focus, active, and color tokens keep targets consistent while the chrome stays visually light. The editor's high-priority Tab/Shift+Tab binding dispatches the same single-level indent/outdent instruction used by the toolbar, retaining focus and preserving one indentation owner. F6 is the separate region route: writing moves to the first detected reference and then to File, Shift+F6 reverses through the last reference, arrow keys move among reference decorations, and Escape returns to writing. The editor gateway owns targeted editor/reference focus instructions while the command deck owns its File focus target; accessible reference names state validity and available actions. The translation picker consumes the normalized catalog's `citationLabel` and full `name`: its collapsed trigger shows only the active abbreviation, while its expanded list supports keyboard navigation and local search across abbreviation, identifier, title, vernacular title, and language code. Selection still enters through the controller; open/query/highlight are ephemeral React view state. A finite command registry supplies labels, shortcuts, enablement, and actions to menus and global keyboard handling.
...
| Scripture | Supply a real browser transport with the existing limits/normalization/fallback policy. Verify DBS CORS from the intended HTTPS origin and whether the existing authorization/cache permission covers that distribution. On 2026-09-06, direct GET probes of the public catalog and ENGWEB/JHN/3 chapter endpoints, with `Origin: https://vercel.com`, returned 200 and `Access-Control-Allow-Origin: *`. This supports trying direct browser requests first; it is not a deployed-origin/browser or distribution-permission proof. If a proxy is necessary, its operations and privacy become an explicit new boundary; it must never receive document prose. |
| Offline loading and updates | Cache the application and bundled WEB through a service worker after an initial successful visit; handle failed installation, schema upgrades, and old/new tabs without losing drafts. The 0.2.3 build emits about 5.40 MB of JavaScript (1.55 MB gzip), so measure first-load cost and separate large static assets where useful. |
...
The owner has clarified the intended later product: a usable browser instance with browser-local drafts and portable document downloads, alongside the Windows installer website. Accounts may be considered only after demand is established. Browser support and acceptable file/PDF semantics still need to be selected before implementation; this reference-correction item adds no web runtime or account/sync/server capability. Shared writing/scripture changes should normally be made once, while adapters, browser/Windows acceptance, and two delivery processes require ongoing target-specific work.

The owner normally uses Vercel. It supports the existing React/Vite frontend, so retain this repository and create an explicit production web build with its own adapters; Vercel must never deploy the deterministic harness. Host the website/editor on Vercel and link the exact immutable installer on GitHub Releases. Before browser implementation lands on the default branch, separate Windows publication from ordinary application pushes: the present Windows workflow treats every non-Markdown push as a release candidate for its configured version. Browser changes must not rebuild or attempt to republish an already published Windows version. Keep the full Windows verification gate behind explicit release intent, with Vercel independently building only the production browser target. This is the proposed delivery shape; no Vercel project has been linked or deployed by the Windows release task. [Vercel Vite documentation](https://vercel.com/docs/frameworks/frontend/vite) checked 2026-09-06.
